...
public class ScheduleListController extends BaseClimateController<ScheduleListController.Callback> implements ScheduleStateController.Callback {
    ...
    ScheduleListController(ModelSource<SubsystemModel> subsystem, AddressableListSource<DeviceModel> thermostats) {
        super(subsystem);
        this.climateDevices = thermostats;
        init();
    }
    ...
    public void init() {
        super.init();
        this.climateDevices.addListener(Listeners.runOnUiThread(new Listener<List<DeviceModel>>() {
            @Override
    ...
    protected void onSubsystemLoaded(ModelAddedEvent event) {
        ArrayList<String> devices = new ArrayList<>(getClimateSubsystem().getControlDevices());
        climateDevices.setAddresses(devices);
        climateDevices.load();
    ...
    protected void onSubsystemCleared(ModelDeletedEvent event) {
        climateDevices.setAddresses(ImmutableList.<String>of());
        super.onSubsystemCleared(event);
    }

    @Override
    protected void updateView(Callback callback, ClimateSubsystem subsystem) {
        if(!climateDevices.isLoaded()) {
            return;
        }

        List<DeviceModel> climateDeviceModels = climateDevices.get();
    ...
    @Override
    public void showScheduleStates(List<ScheduleStateModel> models) {
        if (getCallback() != null) {
    ...
    public interface Callback {
        ...
        /**
         * Show the schedulable device list.  This list will
         * include only thermostat objects.
         * @param thermostats
         */
        void showSchedules(List<ScheduleStateModel> devices);
    }
}
