...
public class ProMonitoringAlarmPanicItemView extends RecyclerView.ViewHolder {
    ...
    private PanicAlarmStatusModel model;

    public ProMonitoringAlarmPanicItemView(View view) {
        super(view);
        cardView = view;
    ...
    public void build(@NonNull PanicAlarmStatusModel model) {
        this.model = model;
        ...
        subText.setText(ArcusApplication.getContext().getString(R.string.alarming));

        dashedCircleView.setAlarmState(DashedCircleView.AlarmState.ALERT);
        alarmIcon.setVisibility(View.GONE);
        alarmStatusText.setText(ArcusApplication.getContext().getString(R.string.alarm_status_ready));
    }
}
