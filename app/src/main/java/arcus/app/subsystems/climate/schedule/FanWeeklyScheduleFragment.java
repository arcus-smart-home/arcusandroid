...
import arcus.app.subsystems.climate.schedule.model.FanCommandFactory;


public class FanWeeklyScheduleFragment extends AbstractWeeklySchedulerFragment implements ScheduleCommandListController.Callbacks {

    private final static String DEVICE_ADDRESS = "DEVICE_ADDRESS";
    private final static String DEVICE_NAME = "DEVICE_NAME";

    public static FanWeeklyScheduleFragment newInstance (String deviceAddress, String deviceName) {
        FanWeeklyScheduleFragment instance = new FanWeeklyScheduleFragment();
    ...
    public void onResume () {
        ...
        ScheduleCommandListController.getInstance().loadScheduledCommandsForDayOfWeek(getDeviceAddress(), getSelectedDayOfWeek(), new FanCommandFactory());
    }

    @Override
    public void onDayOfWeekChanged(DayOfWeek selectedDayOfWeek) {
        ScheduleCommandListController.getInstance().loadScheduledCommandsForDayOfWeek(getDeviceAddress(), selectedDayOfWeek, new FanCommandFactory());
    }

    @Override
    public void onAddCommand() {
        BackstackManager.getInstance().navigateToFragment(FanScheduleCommandEditorFragment.newAddEventInstance(getDeviceAddress(), getDeviceName(), getSelectedDayOfWeek()), true);
    }

    @Override
    public void onEditCommand(Object command) {
        String timeOfDayCommandId = ((ScheduleCommandModel) command).getId();
        BackstackManager.getInstance().navigateToFragment(FanScheduleCommandEditorFragment.newEditEventInstance(getDeviceAddress(), getDeviceName(), timeOfDayCommandId, getSelectedDayOfWeek()), true);
    }
