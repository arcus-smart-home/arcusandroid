...
import arcus.app.subsystems.climate.schedule.model.VentCommandFactory;


public class VentWeeklyScheduleFragment extends AbstractWeeklySchedulerFragment implements ScheduleCommandListController.Callbacks {

    private final static String DEVICE_ADDRESS = "DEVICE_ADDRESS";
    private final static String DEVICE_NAME = "DEVICE_NAME";

    public static VentWeeklyScheduleFragment newInstance (String deviceAddress, String deviceName) {
        VentWeeklyScheduleFragment instance = new VentWeeklyScheduleFragment();
    ...
    public void onResume () {
        ...
        ScheduleCommandListController.getInstance().loadScheduledCommandsForDayOfWeek(getDeviceAddress(), getSelectedDayOfWeek(), new VentCommandFactory());
    }

    @Override
    public void onDayOfWeekChanged(DayOfWeek selectedDayOfWeek) {
        ScheduleCommandListController.getInstance().loadScheduledCommandsForDayOfWeek(getDeviceAddress(), selectedDayOfWeek, new VentCommandFactory());
    }

    @Override
    public void onAddCommand() {
        BackstackManager.getInstance().navigateToFragment(VentScheduleCommandEditorFragment.newAddEventInstance(getDeviceAddress(), getDeviceName(), getSelectedDayOfWeek()), true);
    }

    @Override
    public void onEditCommand(Object command) {
        String timeOfDayCommandId = ((ScheduleCommandModel) command).getId();
        BackstackManager.getInstance().navigateToFragment(VentScheduleCommandEditorFragment.newEditEventInstance(getDeviceAddress(), getDeviceName(), timeOfDayCommandId, getSelectedDayOfWeek()), true);
    }
