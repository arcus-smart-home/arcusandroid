...
class AlertSnackBar private constructor(
...
) : BaseTransientBottomBar<AlertSnackBar>(parent, content, contentViewCallback) {
    private val snackBarOriginalView : View
    init {
        val color = ContextCompat.getColor(context, R.color.sclera_alert)
    ...
    companion object {
        @JvmStatic
        fun make(parent: ViewGroup, @Duration durationMilliSeconds: Int = 7_000) : AlertSnackBar {
            val inflater = LayoutInflater.from(parent.context)
            val view = inflater.inflate(R.layout.snackbar_alert, parent, false)
            val cb   =
                ContentViewCallback()
            val snackBar =
                AlertSnackBar(parent, view, cb)
